...
function socketListener(socket){
    ...
    socket.onopen = function() {
        ...
        document.onkeydown = function KeyCheck(event) {
            parseKeyPress(event.which,socket);

        }

        // key released
        document.onkeyup = function KeyCheck(event) {
            parseKeyRelease(event.which,socket);
        }
    ...
    socket.onclose = function() {
        recieveMessage("Connection closed by server.");
    }
...
function parseKeyPress(key,socket){
...
function parseKeyRelease(key,socket){
